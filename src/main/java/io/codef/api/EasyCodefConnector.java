...
public class EasyCodefConnector {
	...
	protected static EasyCodefResponse execute(String urlPath, int serviceType, HashMap<String, Object> bodyMap, EasyCodefProperties properties) throws InterruptedException {
		...
		if(serviceType == 0) {
			domain = io.codef.api.EasyCodefConstant.API_DOMAIN;
			clientId = properties.getClientId();
			clientSecret = properties.getClientSecret();
		} else if(serviceType == 1) {
			domain = io.codef.api.EasyCodefConstant.DEMO_DOMAIN;
			clientId = properties.getDemoClientId();
			clientSecret = properties.getDemoClientSecret();
		} else {
			domain = io.codef.api.EasyCodefConstant.SANDBOX_DOMAIN;
		...
		try {
			...
			EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.INVALID_JSON);
			return response;
		} catch (UnsupportedEncodingException e) {
			EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.UNSUPPORTED_ENCODING);
			return response;
		}
		
		/**	#3.상품 조회 요청	*/
		HashMap<String, Object> responseMap = requestProduct(domain + urlPath, accessToken, bodyString);
		if(io.codef.api.EasyCodefConstant.INVALID_TOKEN.equals(responseMap.get("error")) || "CF-00401".equals(((HashMap<String, Object>)responseMap.get(io.codef.api.EasyCodefConstant.RESULT)).get(io.codef.api.EasyCodefConstant.CODE))){	// 액세스 토큰 유효기간 만료되었을 경우 토큰 재발급 후 상품 조회 요청 진행
			io.codef.api.EasyCodefTokenMap.setToken(clientId, null);		// 토큰 정보 초기화
			accessToken = getToken(clientId, clientSecret); // 토큰 설정
			responseMap = requestProduct(domain + urlPath, accessToken, bodyString);
		} else if (io.codef.api.EasyCodefConstant.ACCESS_DENIED.equals(responseMap.get("error")) || "CF-00403".equals(((HashMap<String, Object>)responseMap.get(io.codef.api.EasyCodefConstant.RESULT)).get(io.codef.api.EasyCodefConstant.CODE))) {	// 접근 권한이 없는 경우 - 오류코드 반환
			EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.UNAUTHORIZED, io.codef.api.EasyCodefConstant.ACCESS_DENIED);
			return response;
		}
	...
	private static HashMap<String, Object> requestProduct(String urlPath, String token, String bodyString) {
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.BAD_REQUEST, urlPath);
				return response;
			} else if (responseCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
				EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.UNAUTHORIZED, urlPath);
				return response; 
			} else if (responseCode == HttpURLConnection.HTTP_FORBIDDEN) {
				EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.FORBIDDEN, urlPath);
				return response; 
			} else if (responseCode == HttpURLConnection.HTTP_NOT_FOUND) {
				EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.NOT_FOUND, urlPath);
				return response; 
			} else {
				EasyCodefResponse response = new EasyCodefResponse(io.codef.api.EasyCodefMessageConstant.SERVER_ERROR, urlPath);
				return response;
			}
	...
	private static String getToken(String clientId, String clientSecret) throws InterruptedException {
		int i = 0;
		String accessToken = io.codef.api.EasyCodefTokenMap.getToken(clientId);
	...
	protected static HashMap<String, Object> publishToken(String clientId, String clientSecret) {
		...
		try {
			// HTTP 요청을 위한 URL 오브젝트 생성
			URL url = new URL(io.codef.api.EasyCodefConstant.OAUTH_DOMAIN + EasyCodefConstant.GET_TOKEN);
